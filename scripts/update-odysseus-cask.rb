...
UPSTREAM_REPOSITORY = "odysseus-dev/odysseus"
UPSTREAM_BRANCH = "main"
UPSTREAM_URL = "https://github.com/#{UPSTREAM_REPOSITORY}.git"
CASK_PATH = File.expand_path("../Casks/odysseus-source.rb", __dir__)
...
def current_upstream_revision
  ...
  fail_with("git ls-remote returned no #{UPSTREAM_BRANCH} revision") unless revision&.match?(/\A[0-9a-f]{40}\z/)
...
def commit_date(revision, token)
  headers = {
    ...
    "User-Agent" => "medovukha-homebrew-tap",
  }
...
current_revision_match = cask.match(/odysseus_upstream_revision\s*=\s*"([0-9a-f]{40})"/)
fail_with("cask has no odysseus_upstream_revision") if current_revision_match.nil?

latest_revision = current_upstream_revision
if current_revision_match[1] == latest_revision
  puts "Odysseus cask is already at #{latest_revision}."
  exit 0
end
...
updated = cask.dup
updated.sub!(/(odysseus_upstream_revision\s*=\s*")[0-9a-f]{40}(")/) do
